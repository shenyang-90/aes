...
| [8:1] | MODE[7:0] | 工作模式选择 (包含加密/解密、密钥模式等) |
...
// 步骤5: 配置工作模式并启动加密
// MODE[0]=1 (加密), MODE[3:1]=001 (CBC), MODE[5:4]=10 (256-bit)
write_reg(CTRL, 0x13);     // START=1, MODE=0x09 (加密+CBC+256bit)
```
...
##### 8.2.3.4 时钟延迟实现（共因故障防护）

> **设计依据**: [FuSa_Consistency_Check.md - 6.2节 Core B 延迟时钟方案]

为防止同一时钟边沿的共因故障（common-cause failure）同时影响 Core A 和 Core B，采用 **数据延迟锁存方案** 实现时间冗余。

**方案原理**:
- Core A 使用原始时钟，处理原始输入数据
- Core B 使用相同时钟，但输入数据延迟 2 个周期锁存
- Core A 结果延迟对齐后与 Core B 结果进行比较
- 同一时钟边沿的毛刺不会影响两核的同一笔数据
...
module lockstep_with_delay (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            result_a_delay1 <= 128'd0;
            result_a_delay2 <= 128'd0;
            done_a_delay1 <= 1'b0;
            done_a_delay2 <= 1'b0;
        end else begin
...
**方案特点**:
- 同一时钟边沿故障影响时间错开 2 个周期
- 实现简单，时序清晰，符合 Verilog-2001 标准
- 延迟 2 周期平衡防护效果与复杂度（基于 FuSa 建议）
...
#### 8.6.3 BIST 实现（安全机制自检）

> **设计依据**: [FuSa_Consistency_Check.md - 6.2节 安全机制自检 (BIST) 实现方案]

##### 8.6.3.1 BIST 架构概述

安全机制自检 (BIST) 用于验证故障检测逻辑本身的正确性，独立于 DFT 扫描链实现。
...
module safety_bist (
    input  wire        clk,
    input  wire        rst_n,
    input  wire        bist_start,      // 软件触发 (BIST_CTRL[0])
...
localparam TEST_0    = 3'd1;  // Lockstep test
localparam TEST_1    = 3'd2;  // CRC test
localparam TEST_2    = 3'd3;  // Timeout test
localparam COMPLETE  = 3'd4;
localparam FAILED    = 3'd5;
        ...
        case (test_state)
            ...
            TEST_2: begin
                ...
                if (timeout_flag) begin
                    ...
                    bist_done <= 1'b1;
                    test_state <= COMPLETE;
                end else if (test_cnt > 16'd5000) begin  // Timeout threshold
                    bist_pass <= 1'b0;
            ...
            COMPLETE: begin
                // 保持完成状态直到 bist_start 释放
                if (!bist_start)
                    test_state <= IDLE;
            end
            
            FAILED: begin
                // 保持失败状态直到 bist_start 释放
                fi_enable <= 1'b0;
                if (!bist_start)
                    test_state <= IDLE;
            end
...
// 测试模式下的故障注入掩码
wire [127:0] fi_mask;
assign fi_mask = test_en ? gen_fault_mask(test_target) : 128'd0;

// 测试模式下的故障注入
wire [127:0] result_b_test;
assign result_b_test = result_b ^ fi_mask;

// 正常比较逻辑（使用可能被注入故障的 result_b_test）
assign fault_detected = enable && (result_a != result_b_test);

// 输出监控
assign fault_det_out = fault_detected;
assign int_fault_out = fault_detected;

// 故障掩码生成函数
function [127:0] gen_fault_mask;
    input [7:0] target;
    begin
        case (target)
            8'h01: gen_fault_mask = 128'h1;           // Bit 0 flip
            8'h02: gen_fault_mask = 128'hFFFFFFFF;    // Word flip
            8'h04: gen_fault_mask = 128'hFFFFFFFFFFFFFFFF; // Double word flip
            default: gen_fault_mask = 128'h0;
        endcase
    end
endfunction
...
| 寄存器 | 地址 | 位 | 名称 | 访问 | 描述 |
|--------|------|-----|------|------|------|
| BIST_CTRL | 0x50 | [0] | START | RW | 启动 BIST |
| | | [1] | MODE | RW | 0=上电自检, 1=周期自检 |
| BIST_STATUS | 0x54 | [0] | DONE | RO | BIST 完成 |
| | | [1] | PASS | RO | 1=通过, 0=失败 |
| | | [4:2] | FAIL_ID | RO | 失败的测试项 ID |
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        bist_ctrl_reg <= 32'd0;
    end else if (apb_write && paddr == BIST_CTRL_ADDR) begin
        bist_ctrl_reg <= pwdata;
    end
end
...
> **设计依据**: [Safety_Mechanism_Signals.md - 第2节 安全机制信号列表]

##### 8.8.2.1 故障类型编码

| 故障类型 | 编码 (fault_type[2:0]) | 检测方式 | 触发条件 | 对应信号 |
|----------|------------------------|----------|----------|----------|
| **结果不匹配** | 3'b000 | result_a ≠ result_b | Core A/B 输出不一致 | fault_detected |
| **CRC错误** | 3'b001 | crc_mismatch | CRC校验失败 | crc_error |
| **超时错误** | 3'b010 | timeout_expired | 状态机卡住 | timeout_flag |
| **奇偶错误** | 3'b011 | parity_mismatch | 寄存器奇偶校验失败 | parity_error |
| **模式错误** | 3'b100 | mode_invalid | 无效操作模式 | mode_err |
| **密钥错误** | 3'b101 | key_invalid | 密钥加载异常 | key_err |
| **配置错误** | 3'b110 | cfg_mismatch | 配置寄存器位翻转 | cfg_err |
| **保留** | 3'b111 | - | - | - |
...
| 信号名 | 来源模块 | 位宽 | 描述 | STATUS 寄存器位 |
|--------|----------|------|------|-----------------|
| fault_detected | fault_detector | 1 | 故障检测标志 | STATUS[4] |
| dual_err | fault_detector | 1 | 双轨比较错误 | STATUS[3] |
| crc_err | crc_checker | 1 | CRC校验错误 | STATUS[2] |
| timeout_err | timeout_monitor | 1 | 超时错误 | STATUS[6] |
| parity_err | parity_checker | 1 | 奇偶校验错误 | STATUS[7] |
| mode_err | mode_controller | 1 | 模式错误 | STATUS[8] |
| key_err | key_manager | 1 | 密钥错误 | STATUS[9] |

**中断信号映射**:

| 中断信号 | 中断源 | INT_STATUS 位 | INT_EN 位 | 清除方式 |
|----------|--------|---------------|-----------|----------|
| int_fault | fault_detector | [2] | [2] | W1C |
| int_crc | crc_checker | [3] | [3] | W1C |
| int_timeout | timeout_monitor | [4] | [4] | W1C |
| int_done | controller | [1] | [1] | W1C |
| int_error | controller | [0] | [0] | W1C |
...
1. 状态机进入某状态
2. 超过预定时间未转移
3. timeout_monitor 触发
   ↓
4. fault_type = 3'b010 (timeout)
5. 强制状态机返回 IDLE
6. STATUS[TIMEOUT_ERR] = 1
7. INT_STATUS[TIMEOUT_STATUS] = 1
```

##### 8.8.2.4 故障注入验证映射

根据 [Safety_Mechanism_Signals.md - 第3节 注错场景]:

| 场景ID | 故障类型 | 注入信号 | 预期 fault_type | 验证状态 |
|--------|----------|----------|-----------------|----------|
| FI-FD-001 | 单比特翻转 | result_a[0] | 3'b000 | 待验证 |
| FI-FD-002 | 单比特翻转 | result_a[63] | 3'b000 | 待验证 |
| FI-FD-007 | 单比特翻转 | result_b[0] | 3'b000 | 待验证 |
| FI-CRC-001 | CRC错误 | data_in[0] | 3'b001 | 待验证 |
| FI-CRC-002 | 多比特错 | data_in[31:0] | 3'b001 | 待验证 |
| FI-CTL-001 | FSM卡住 | state | 3'b010 | 待验证 |
| FI-KEY-004 | 密钥清零 | zeroize | 3'b101 | 待验证 |
...
void aes_fault_isr() {
    ...
    if (int_status & 0x04) {  // FAULT_STATUS at bit[2]
        // Log fault event
        ...
        // Clear fault status
        write_reg(INT_STATUS, 0x04);  // Clear FAULT_STATUS
...
> **设计依据**: [Safety_Mechanism_Signals.md - 第5-6节 信号验证与检查清单]
...
| 信号类别 | 验证方法 | 检查点 | 通过标准 |
|----------|----------|--------|----------|
| **时钟信号** | 时钟监控 | clk 频率/占空比 | 在规格范围内 |
| **复位信号** | 复位序列 | rst_n 释放时序 | 同步释放，无毛刺 |
| **数据信号** | 逻辑分析仪 | result_a/b 对齐 | 延迟匹配设计值 |
| **控制信号** | 断言检查 | enable/valid 时序 | 满足建立/保持时间 |
| **中断信号** | 中断监控 | int_fault 脉冲宽度 | ≥1 时钟周期 |

##### 8.10.1.2 安全机制激活验证

```verilog
// 断言验证模板
`ifdef SAFETY_ASSERTIONS
...
assert property (
    ...
    |-> ##1 fault_detected
) else $error("AS1 failed: fault_detected not asserted on mismatch");

// AS2: safe_result 必须在无故障时等于 result_a
assert property (
    @(posedge clk) disable iff (!rst_n)
    (!fault_detected && result_a_valid) 
    |-> (safe_result == $past(result_a))
) else $error("AS2 failed: safe_result mismatch when no fault");

// AS3: 故障时必须输出零
assert property (
    @(posedge clk) disable iff (!rst_n)
    fault_detected |-> (safe_result == 128'h0)
) else $error("AS3 failed: safe_result not zero on fault");

// AS4: 中断必须在故障检测后置位
assert property (
    @(posedge clk) disable iff (!rst_n)
    fault_detected |-> ##1 int_fault
) else $error("AS4 failed: interrupt not generated");
...
```verilog
module safety_verification_if (
    input  wire        clk,
    input  wire        rst_n,
    
    // 连接到被测模块的信号
    input  wire [127:0] result_a,
    input  wire [127:0] result_b,
    input  wire         fault_detected,
    input  wire [127:0] safe_result,
    
    // 验证控制接口
    input  wire         verify_mode,     // 1=验证模式
    input  wire [7:0]   fi_select,       // 故障选择
    input  wire         fi_enable,       // 故障使能
    output reg          verify_pass,     // 验证通过
    output reg  [3:0]   verify_status    // 验证状态
);

// 故障注入控制
reg [127:0] result_a_fi;
reg [127:0] result_b_fi;

always @(*) begin
    if (verify_mode && fi_enable) begin
        case (fi_select)
            8'h01: result_a_fi = result_a ^ 128'h1;           // Bit 0 flip
            8'h02: result_a_fi = result_a ^ 128'hFF;          // Byte flip
            8'h04: result_a_fi = result_a ^ 128'hFFFFFFFF;    // Word flip
            8'h08: result_a_fi = 128'h0;                      // All zero
            8'h10: result_a_fi = {128{1'b1}};                 // All one
            8'h20: result_b_fi = result_b ^ 128'h1;           // Flip result_b
            default: begin
                result_a_fi = result_a;
                result_b_fi = result_b;
            end
        endcase
    end else begin
        result_a_fi = result_a;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        verify_pass <= 1'b0;
        verify_status <= 4'h0;
    end else if (verify_mode) begin
        // 检查故障检测响应
        if (fi_enable && (result_a_fi != result_b_fi)) begin
            if (fault_detected && safe_result == 128'h0)
                verify_pass <= 1'b1;
            else
                verify_status <= 4'h1;  // 故障未正确检测
        end
    end
end
...
> **来源**: [Safety_Mechanism_Signals.md - 第3节 注错场景详细列表]

##### 8.10.2.1 Fault Detector 测试场景

| 场景ID | 故障类型 | 注入信号 | 注入值 | 预期检测信号 | 预期行为 |
|--------|----------|----------|--------|--------------|----------|
| FI-FD-001 | 单比特翻转 | result_a[0] | ~result_a[0] | fault_detected=1 | 进入ERROR状态 |
| FI-FD-002 | 单比特翻转 | result_a[63] | ~result_a[63] | fault_detected=1 | 进入ERROR状态 |
| FI-FD-003 | 单比特翻转 | result_a[127] | ~result_a[127] | fault_detected=1 | 进入ERROR状态 |
| FI-FD-004 | 多比特翻转 | result_a[31:0] | ~result_a[31:0] | fault_detected=1 | 进入ERROR状态 |
| FI-FD-005 | 全零注入 | result_a | 128'h0 | fault_detected=1 | 进入ERROR状态 |
| FI-FD-006 | 全一注入 | result_a | 128'hFFFF... | fault_detected=1 | 进入ERROR状态 |
| FI-FD-007 | 单比特翻转 | result_b[0] | ~result_b[0] | fault_detected=1 | 进入ERROR状态 |
| FI-FD-008 | 结果A有效丢失 | result_a_valid | 1'b0 | state stuck | 超时检测 |
| FI-FD-009 | 结果B有效丢失 | result_b_valid | 1'b0 | state stuck | 超时检测 |
| FI-FD-010 | CRC错误注入 | crc_valid | 1'b0 | fault_detected=1, fault_type=1 | 进入ERROR状态 |

##### 8.10.2.2 CRC Checker 测试场景

| 场景ID | 故障类型 | 注入信号 | 注入值 | 预期检测信号 | 预期行为 |
|--------|----------|----------|--------|--------------|----------|
| FI-CRC-001 | 数据单比特错 | data_in[0] | ~data_in[0] | crc_out变化 | CRC值改变 |
| FI-CRC-002 | 数据多比特错 | data_in[31:0] | ~data_in[31:0] | crc_out变化 | CRC值改变 |
| FI-CRC-003 | 全零数据 | data_in | 128'h0 | crc_out=0xFFFFFFFF | 特定CRC值 |
| FI-CRC-004 | 全一数据 | data_in | 128'hFFFF... | crc_out=计算值 | 验证多项式 |
| FI-CRC-005 | Calc stuck | calc_start | 1'b0 | crc_valid=0 | 无CRC输出 |
...
> **来源**: [Safety_Mechanism_Signals.md - 第6节 安全机制验证检查清单]

| 检查项 | 验证方法 | 目标 | 状态 |
|--------|----------|------|------|
| **Dual-rail compare** | 故障注入 | 能检测所有单比特翻转 | ☐ |
| **Dual-rail compare** | 故障注入 | 能检测所有多比特翻转 | ☐ |
| **CRC check** | 故障注入 | 能检测所有单比特数据错误 | ☐ |
| **CRC check** | 故障注入 | 能检测所有双比特数据错误 | ☐ |
| **CRC check** | 故障注入 | 能检测所有奇数比特错误 | ☐ |
| **CRC check** | 故障注入 | 能检测所有突发错误 < 32bit | ☐ |
| **Key zeroize** | 功能测试 | 能在一个周期内清零所有密钥位 | ☐ |
| **Key zeroize** | 优先级测试 | 优先级高于 key_load | ☐ |
| **FSM timeout** | 故障注入 | 能检测所有状态卡住 | ☐ |
| **FSM** | 非法状态测试 | 无效状态能安全回到IDLE | ☐ |
| **Interrupt** | 功能测试 | 所有中断能正确触发和清除 | ☐ |
| **Interrupt** | 使能测试 | 中断使能/屏蔽功能正常 | ☐ |
| **Fault response** | 端到端测试 | 故障检测后能正确进入ERROR状态 | ☐ |
| **Safe output** | 故障注入 | ERROR状态输出被强制为零 | ☐ |

#### 8.10.4 覆盖率目标

| 覆盖类型 | 目标值 | 检查点 | 验证阶段 |
|----------|--------|--------|----------|
| 安全机制激活率 | 100% | 所有 fault_detected 触发场景 | 仿真/FPGA |
| FSM状态覆盖 | 100% | fault_detector 所有7个状态 | 仿真 |
| 中断触发 | 100% | 所有中断类型 | 仿真/FPGA |
| 故障类型覆盖 | 100% | 8种 fault_type 编码 | 仿真 |
| 关键信号翻转 | >95% | result/key/data 信号 | 故障注入 |
| 寄存器访问 | 100% | 所有APB寄存器 | 仿真 |
...
| | | - 修正表3.2 (CTRL): MODE[8:1] + DUAL_RAIL_EN[9] | |
| | | - 修正表3.3 (STATUS): BUSY[0] + STATE[3:1] + FAULT_DETECTED[4] | |
| | | - 修正表3.8 (INT_EN): ERROR_INT_EN[0] + DONE_INT_EN[1] + FAULT_INT_EN[2] | |
| | | - 修正表3.9 (INT_STATUS): ERROR_STATUS[0] + DONE_STATUS[1] + FAULT_STATUS[2] | |
| | | - 更新8.7节寄存器定义，与FuSa检查报告一致 | |
| | | - 更新示例代码中的寄存器位引用 | |
| **v1.0** | **2026-04-01** | **FuSa 驱动更新: 根据 FuSa 文档完善设计规格** | **Design Agent** |
| | | - **新增 8.2.3.4 时钟延迟实现**: Core B 延迟数据锁存方案，防止共因故障 | |
| | | - **新增 8.6.3 BIST 实现**: 安全机制自检架构，含故障注入接口 | |
| | | - **更新 8.8.2 故障类型编码**: 3-bit 编码与信号映射，对齐 Safety_Mechanism_Signals.md | |
| | | - **新增 8.10 安全机制信号验证方案**: 信号验证方法、故障注入场景、检查清单 | |
| | | - **新增 BIST 寄存器**: BIST_CTRL (0x50), BIST_STATUS (0x54) | |
| | | - **完善故障响应流程**: 8种故障类型的检测与响应机制 | |
| | | - **新增安全机制断言模板**: AS1-AS4 断言检查 | |
| | | - **所有新增内容引用 FuSa 文档**: 明确设计依据 | |
...
- [Safety_Mechanism_Signals.md](../FuSa/Safety_Mechanism_Signals.md) - 安全机制信号分析
- [FuSa_Consistency_Check.md](../FuSa/FuSa_Consistency_Check.md) - FuSa一致性检查
- [FMEDA_Report.md](../FuSa/FMEDA_Report.md) - FMEDA分析报告
...
- [Architecture_Spec.md](../Arch/Architecture_Spec.md) - 架构规格书
�书
ategy.md](./CDC_Strategy.md) - CDC策略文档
- [Architecture_Spec.md](../Arch/Architecture_Spec.md) - 架构规格书
md) - CDC策略文档
- [Architecture_Spec.md](../Arch/Architecture_Spec.md) - 架构规格书
d) - 架构规格书
